...
if (CLIENT) then
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/throwinghatchet.mdl", bone = "Bip01 R Hand", rel = "", pos = Vector(3.595, -1.31, 0.674), angle = Angle(-7.252, -89.909, 0.503), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/throwinghatchet.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(1.753, 1.504, -1.275), angle = Angle(180, -90.003, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
end

SWEP.Author					= "Halokiller38"
...
function SWEP:PrimaryAttack()
	...
	if (SERVER) then
		local knife = ents.Create("ent_halo_hatchet")
...
////A little something that I added so that if the player throws their knife and has no knives remaining, the SWEP itself is removed until they pick up one of their knives off the ground, which makes it more R34L1571C//
		if(self.Weapon:IsValid() && self.Owner:IsValid()) then
			if (self.Owner:GetAmmoCount(self.Primary.Ammo) < 1) then
				self.Owner:StripWeapon("weapon_throwinghatchet")
			end
		end
