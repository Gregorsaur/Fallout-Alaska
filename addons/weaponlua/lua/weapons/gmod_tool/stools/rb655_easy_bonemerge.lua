...
--[[
	TODO: Limit bonemerge depth or fix the lag from doing so
]]
...
--[[
local maxDepthConvar
local maxMergesConvar
if ( SERVER ) then
	maxDepthConvar = CreateConVar( "rb655_easy_bonemerge_maxdepth", "5", FCVAR_ARCHIVE )
	maxMergesConvar = CreateConVar( "rb655_easy_bonemerge_maxmerges", "10", FCVAR_ARCHIVE )
end

cleanup.Register( "bonemerges" )

local function CountEntitysBonemergeDepth( ent, d )
	local c = d or 1
	for id, e in pairs( ent:GetChildren() ) do
		if ( e:GetClass() == "ent_bonemerged" ) then
			c = math.max( c, CountEntitysBonemergeDepth( e, (d or 0) + 1 ) )
		end
	end
	return c
end
local function CountEntitysBonemergeAmount( ent )
	local c = 0
	for id, e in pairs( ent:GetChildren() ) do
		if ( e:GetClass() == "ent_bonemerged" ) then c = c + 1 end
	end
	return c
end
]]
...
function TOOL:LeftClick( tr )
	...
	--[[print( "side-by-side", CountEntitysBonemergeAmount( ent ) )
	print( "depth", CountEntitysBonemergeDepth( ent ) )
	if ( GetConVarNumber( "rb655_easy_bonemerge_maxmerges" ) < CountEntitysBonemergeAmount( ent ) ) then
		// error message
		return false
	end]]
