...
if ( CLIENT ) then
	language.Add( "tool.rt_camera.name", "RT Camera" )
	language.Add( "tool.rt_camera.desc", "Displays things through an RT Camera" )
	language.Add( "tool.rt_camera.0", "Left click to place a camera, Right click to place a camera that looks at you" )
	language.Add( "tool.rt_camera.help", "Displays things through an RT Camera" )
	language.Add( "tool.rt_camera.key", "RT Camera Key" )
	language.Add( "tool.rt_camera.locked", "Static Camera" )
	language.Add( "tool.rt_camera.locked.help", "Makes the camera unmovable." )
	language.Add( "tool.rt_camera.draw", "Draw RT Window" )
	language.Add( "tool.rt_camera.draw.help", "Displays the RT Camera window on the player's screen. Use the RT Window button to change its position and size.")
	language.Add( "tool.rt_camera.window", "RT Window" )
	language.Add( "tool.rt_camera.window.help", "Drag the resize corner to resize. Click the upper right corner to stop editing." )
end
...
function TOOL.BuildCPanel( CPanel )
	CPanel:AddControl( "Header", { Description = "#tool.rt_camera.help" } )
	CPanel:AddControl( "Numpad", { Label = "#tool.rt_camera.key", Command = "rt_camera_key"} )
	CPanel:AddControl( "CheckBox", { Label = "#tool.rt_camera.locked", Command = "rt_camera_locked", Help=true }  )
	CPanel:AddControl( "CheckBox", { Label = "#tool.rt_camera.draw", Command = "rt_camera_draw", Help=true }  )
	CPanel:AddControl( "Button", { Label = "#tool.rt_camera.window", Command = "rt_camera_window", Help=true } )
end

if (CLIENT) then

local rt_camera_draw = CreateClientConVar( "rt_camera_draw", "0", false, false )

local RTWindow = nil

local function OpenRTWindow( player, command, arguments )

	LocalPlayer():ConCommand( "rt_camera_draw 1" )
...
concommand.Add( "rt_camera_window", OpenRTWindow )
...
local function DrawRTTexture()
	...
	if ( !rt_camera_draw:GetBool() ) then
