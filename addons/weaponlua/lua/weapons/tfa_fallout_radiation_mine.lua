SWEP.Gun = ("tfa_fallout_radiation_mine") -- must be the name of your swep but NO CAPITALS!
...
SWEP.Primary.Ammo		 = "Radiationmine"
SWEP.Primary.Round 	     = ("tfa_fallout_radiation_mine")  
SWEP.Velocity            = 50
...
SWEP.WorldModel			 = "models/weapons/tfa_fallout/w_fallout_cryomine.mdl"
	...
	timer.Simple( plant, function() if not IsValid( self ) then return end if IsValid( self.Owner ) and IsValid( self.Weapon ) then
		...
		if self.Owner:Alive() and self.Owner:GetActiveWeapon():GetClass() == self.Gun then
			self.Weapon:SendWeaponAnim( ACT_VM_SECONDARYATTACK )
				...
				proxy = ents.Create("fallout_radiation_mine_explosive_ent")
