...
if (CLIENT) then
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/protonaxe.mdl", bone = "Bip01 R Hand", rel = "", pos = Vector(5.234, 3.339, 0.416), angle = Angle(0.3, -74.137, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/protonaxe.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(4.084, -1.859, 1.284), angle = Angle(180, -106.765, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
end
...
function SWEP:PrimaryAttack()
	...
	if (SERVER) then
		local knife = ents.Create("ent_halo_protonaxe")
...
////A little something that I added so that if the player throws their knife and has no knives remaining, the SWEP itself is removed until they pick up one of their knives off the ground, which makes it more R34L1571C//
		if(self.Weapon:IsValid() && self.Owner:IsValid()) then
			if (self.Owner:GetAmmoCount(self.Primary.Ammo) < 1) then
				self.Owner:StripWeapon("weapon_throwingprotonaxe")
			end
		end
