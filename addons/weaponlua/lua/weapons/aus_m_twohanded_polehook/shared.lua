...
SWEP.ParrySound = Sound("weapons/katanasounds/katana_swing_miss1.wav")
...
local HitSoundWorld = Sound( "weapons/dagger/skyrim_dagger_hitwall2.mp3" )
local HitSoundBody = Sound( "weapons/greatsword/skyrim_greatsword_flesh1.mp3" )
...
//This if shot the bullets
	...
	if ( tr.Hit ) then
		...
		bullet.Damage = math.random( 50, 60 )
...
function SWEP:PrimaryAttack()
	...
	if self:Ammo1() < 5 then return end
			...
			timer.Simple( 0.3, function() if self:IsValid() then
	...
	timer.Create("hitdelay", 0.60, 1, function() self:Hitscan() end)
...
SWEP.VElements = {
	["v_polehook"] = { type = "Model", model = "models/mosi/fallout4/props/weapons/melee/polehook.mdl", bone = "RightHand_1stP", rel = "", pos = Vector(-3.5, 4.675, 0.518), angle = Angle(100, 36.234, -125.066), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {[0] = 1} },
...
SWEP.WElements = {
	["w_polehook"] = { type = "Model", model = "models/mosi/fallout4/props/weapons/melee/polehook.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3, 1.557, 2.596), angle = Angle(180, 64.286, 1.169), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {[0] = 1} }
}