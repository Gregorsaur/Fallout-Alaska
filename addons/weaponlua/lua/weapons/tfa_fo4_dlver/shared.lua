...
SWEP.Category				= "TFA Fallout 4" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep.
...
SWEP.PrintName				= "The Deliverer"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("TFA_FO4_DELIVERER.2")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound 			= Sound("TFA_FO4_DELIVERER.1")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.SpreadRecovery = 7.5 --How much the spread recovers, per second.
...
SWEP.MoveSpeed = 240/250 --Multiply the player's movespeed by this.
SWEP.IronSightsMoveSpeed  = 240/250*0.8 --Multiply the player's movespeed by this when sighting.
...
SWEP.BlowbackVector = Vector(0,-1.5,0.05) --Vector to move bone <or root> relative to bone <or view> orientation.
SWEP.BlowbackCurrentRoot = 0 --Amount of blowback currently, for root
SWEP.BlowbackCurrent = 0 --Amount of blowback currently, for bones
SWEP.BlowbackBoneMods = {
	["v_weapon.Slide"] = { scale = Vector(1, 1, 1), pos = Vector(-0, -0, -1.173), angle = Angle(0, 0, 0) },
	["v_weapon.223_parent"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 0, -2) },
	["v_weapon.Bip01_R_Hand"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(1, 0, 0) }
}
...
SWEP.Callback.Deploy = function(self)
	if self:OwnerIsValid() then
		...
		self:SetBodygroup(1,self:GetSilenced() and 1 or 0)
	end
end

SWEP.Callback.Think2 = nil
...
SWEP.Callback.PlayerThink = function(self)
	...
	if !self.LastSilenced then
		self:SetBodygroup(1,self.Silenced and 1 or 0)
		self.LastSilenced = self.Silenced
	end
	
	if self.LastSilenced != self.Silenced then
		self:SetBodygroup(1,self.Silenced and 1 or 0)
	end
	...
	self:UpdateViewModelSilencer()
	
	self.MuzzleFlashEffect = self.Silenced and "tfa_muzzleflash_silenced" or "tfa_muzzleflash_pistol"
end
...
SWEP.Callback.PlayerThinkClientFrame = function(self,ply)
	...
	if IsValid(ply) and !ply.ShouldDrawLocalPlayer or ply:ShouldDrawLocalPlayer() then
		self:SetBodygroup(1,self.Silenced and 1 or 0)
	end
...
SWEP.ViewModelBoneMods = {
	["v_weapon.silencer"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) }
}

function SWEP:UpdateViewModelSilencer()
	local ow = self.Owner
	if !IsValid(ow) then return end
	local vm = self.Owner:GetViewModel()
	if !IsValid(vm) then return end
	local seq = vm:GetSequenceActivity(vm:GetSequence())
	
	local dosil = ( self.Silenced )--or ( (seq==ACT_VM_ATTACH_SILENCER or seq==ACT_VM_DETACH_SILENCER) and vm:GetCycle()<0.995 ) )
	
	if seq==ACT_VM_ATTACH_SILENCER then
		if vm:GetCycle()>0.01 then dosil = true else dosil = false end
	end
	
	if seq==ACT_VM_DETACH_SILENCER then
		if vm:GetCycle()<0.99 then dosil = true else dosil = false end
	end
	
	self.ViewModelBoneMods["v_weapon.silencer"].pos.y = dosil and 0 or -300
end
