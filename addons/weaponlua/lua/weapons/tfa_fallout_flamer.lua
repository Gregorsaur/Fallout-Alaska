...
SWEP.Author				    = "Yo momma"              -- Author Tooltip
...
SWEP.Primary.Velocity          = nil           -- Bullet Velocity in m/s
...
SWEP.Primary.Range        = 25 * 25               -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
SWEP.InspectPos     = Vector(5.5, -5, -2.5) 
SWEP.InspectAng     = Vector(24.622, 42.915, 15.477)

--[[ VIEWMODEL ]]--

SWEP.ViewModel		= "models/weapons/tfa_fallout/c_fallout_flamer.mdl" -- Viewmodel path
...
SWEP.WorldModel	    = "models/weapons/tfa_fallout/w_fallout_flamer.mdl" -- Weapon world model path
...
function SWEP:Think2( ... )
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			self:EmitSound("Fallout_Weapon_Flamer.in")
			...
			self:EmitSound("Fallout_Weapon_Flamer.end")
	...
	if shooting then
		if self.NextIdleSound and CurTime() > self.NextIdleSound then
			self:EmitSound("Fallout_Weapon_Flamer.loop")
			self.NextIdleSound = CurTime() + SoundDuration( "Fallout_Weapon_Flamer.loop" ) - 0.1
		end
	end
