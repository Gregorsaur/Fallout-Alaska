...
ENT.AttachmentInfo = {
	...
	OffsetVec = Vector( -7 , -7.5 , -0.5 ),
	OffsetAng = Angle( 175 , 100 , .6 ),
}
...
function ENT:Initialize()
	...
	if SERVER then
		self:SetModel( "models/fallout/weapons/c_l30_backpack.mdl" )
		self:InitPhysics()
		...
		self:SetFuelDrain(1.5)	--drain in seconds
		self:SetFuelRecharge( 30 )	--recharge in seconds
		self:SetActive( false )
		self:SetCanStomp( false )
		self:SetDoGroundSlam( false )
		self:SetAirResistance( 2.5 )
		self:SetRemoveGravity( true )
		self:SetJetpackSpeed( 300 )
		self:SetJetpackStrafeSpeed( 300 )
		self:SetJetpackVelocity( 650 )
		self:SetJetpackStrafeVelocity( 35 )
	else
