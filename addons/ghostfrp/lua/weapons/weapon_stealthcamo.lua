...
function SWEP:Cloak( pl )
	...
	for i,v in pairs(self:GetOwner().parts or {} )  do
		if IsValid(v) then v:DrawShadow( false ) v:SetNoDraw(true) end
	end
end
...
function SWEP:Uncloak( pl )
	...
	self.Owner:DrawShadow( true ) 
		for i,v in pairs(self:GetOwner().parts or {} )  do
		if IsValid(v) then v:DrawShadow( true )  v:SetNoDraw(false) end
	end
end
...
if SERVER then
	...
	function SWEP:PrepareMaterial( mat )
		...
		Materials[ mat ] = CreateMaterial( mat .. "-cloxaeaked", shader, params )
	end
	...
	function SWEP:CloakThink()
		...
		for i,v in pairs(self.Owner.parts or {}) do 
			v.CloakFactor=self.Owner.CloakFactor
		end
	end
	...
	function SWEP:PrePlayerDraw( pl )
		...
		for k, v in ipairs( self.Owner:GetMaterials() ) do
			...
			render.MaterialOverrideByIndex( k - 1, Materials[ v ] )
			  ...
			  for i, z in pairs(self.Owner.parts or {}) do
 
        if IsValid(z) then 
	

		for k, v in ipairs(z and z:GetMaterials() or {}) do
			if not Materials[ v ] then self:PrepareMaterial( v ) end
			render.MaterialOverrideByIndex( k - 1, Materials[ v ] )
...
timer.Create("dc",0.01,10,function() for i,v in pairs(player.GetAll()) do end end)