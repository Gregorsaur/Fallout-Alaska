...
function ENT:canAccess(client)
	if (table.HasValue(VENDOR_MODIFY_ACCESS or {}, client:GetUserGroup())) then
		return true
	end
...
if (SERVER) then
	...
	function ENT:Use(activator)
		...
		-- Only send what is needed.
		for k, v in pairs(self.items) do
			if (table.Count(v) > 0 and (table.HasValue(VENDOR_MODIFY_ACCESS or {}, activator:GetUserGroup()) or v[VENDOR_MODE])) then
				items[k] = v
			end
		end
		...
		if (table.HasValue(VENDOR_MODIFY_ACCESS or {}, activator:GetUserGroup())) then
			data[4] = self.messages
			data[5] = self.factions
			data[6] = self.classes
		end
