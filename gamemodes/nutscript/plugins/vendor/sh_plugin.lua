...
VENDOR_SELL = 2
VENDOR_BOTH = 3
VENDOR_MODIFY_ACCESS = { "developer", "founder", "superadmin", "communitymanager", "manager" }
		...
		local data = {}
			for k, v in ipairs(ents.FindByClass("nut_vendor")) do
				...
				for i=0, v:GetNumBodyGroups()-1 do
					bodygroupsString = bodygroupsString .. v:GetBodygroup(i)
				end
	...
	function PLUGIN:LoadData()
		for k, v in ipairs(self:getData() or {}) do
			...
			if (v.bodygroups) then
				entity:SetBodyGroups(v.bodygroups)
			end
		end
	end
	...
	netstream.Hook("vendorTrade", function(client, uniqueID, isSellingToVendor)
		...
		if (entity.items[uniqueID] and hook.Run("CanPlayerTradeWithVendor", client, entity, uniqueID, isSellingToVendor) != false) then
			...
			if (isSellingToVendor) then
				...
				if (mLogs) then mLogs.log("vendor","nut", {client=mLogs.logger.getPlayerData(client), vendor=entity:getNetVar("name", "N/A"), item=nameENG, selling=isSellingToVendor, price=tostring(price)}) end
			else
				...
				local item = client:getChar():getInv():add(uniqueID, 1, {boughtFor = price})
				if (!item) then
					local ply = client
					
					if (ply.LastItemDrop && SysTime() - ply.LastItemDrop < 2) then
						ply:notify("You must wait before spawning something again!")
						return false
					end
					
					local count = 0
					
					ply.ItemDrops = ply.ItemDrops or {}

					for k,v in pairs(ply.ItemDrops) do
						if (SysTime() - v < 600) then
							count = count + 1
						else
							ply.ItemDrops[k] = nil
						end
					...
					ply.ItemDrops = table.ClearKeys(ply.ItemDrops)

					
					if (count >= 10) then
						ply:notify("You have reached your limit for items spawned in 10 minutes!")
						return false
					end
					
					table.insert(ply.ItemDrops, SysTime())
					ply.LastItemDrop = SysTime()
					
					item = nut.item.spawn(uniqueID, client:getItemDropPos(), nil, nil, {boughtFor = price})
				else
					netstream.Start(client, "vendorAdd", uniqueID)
				end

				local name = L(nut.item.list[uniqueID].name, client)
				local nameENG = nut.item.list[uniqueID].name
